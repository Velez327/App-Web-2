...
func nuevoRouterCompleto(t *testing.T, db *gorm.DB) http.Handler {
	...
	return handlers.NuevoRouter(
		handlers.NuevoMedicamentoHandler(services.NuevoMedicamentoService(medicamentoRepo)),
		handlers.NuevoClienteHandler(services.NuevoClienteService(clienteRepo)),
		handlers.NuevaVentaHandler(services.NuevaVentaService(ventaRepo, medicamentoRepo, clienteRepo)),
	)
}
